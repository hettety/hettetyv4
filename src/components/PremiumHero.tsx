  ...
  return (
    <div className="relative min-h-[90vh] flex items-center justify-center overflow-hidden bg-white dark:bg-[#0F172A] font-sans transition-colors duration-500" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-brand-500/10 dark:from-brand-500/20 via-transparent to-transparent"></div>
      </div>
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
        ...
        <div className="inline-flex items-center gap-2 bg-slate-900/5 dark:bg-white/5 backdrop-blur-md border border-slate-900/10 dark:border-white/10 rounded-full px-5 py-2 mb-8 animate-slide-up shadow-sm">
          ...
          <span className={`text-brand-600 dark:text-accent-400 text-xs font-black ${isRtl ? 'font-cairo' : 'tracking-widest uppercase'}`}>
            {t.hero_badge}
          </span>
        </div>

        {/* Main Title */}
        <h1 className={`text-5xl md:text-7xl lg:text-9xl font-black text-[#1b2c4d] dark:text-white mb-8 leading-[0.9] animate-fade-in ${isRtl ? 'font-cairo' : 'font-heading tracking-tighter uppercase'}`}>
        ...
        {/* Subtitle */}
        <p className={`text-lg md:text-2xl text-slate-600 dark:text-slate-300 mb-12 max-w-3xl mx-auto leading-relaxed font-bold animate-slide-up opacity-90 ${isRtl ? 'font-cairo' : 'tracking-tight uppercase'}`}>
          {t.hero_subtitle}
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-center items-center animate-slide-up" style={{ animationDelay: '0.2s' }}>
          ...
          <button 
            ...
            className={`group relative px-10 py-5 bg-accent-500 hover:bg-accent-600 text-white text-xl font-black rounded-2xl transition-all duration-300 shadow-2xl shadow-accent-500/20 hover:shadow-accent-500/40 transform hover:-translate-y-1 w-full sm:w-auto flex items-center justify-center gap-2 overflow-hidden ${isRtl ? 'font-cairo' : 'uppercase tracking-widest'}`}
          >
