...
const CookiePolicyPage: React.FC<CookiePolicyPageProps> = ({ t, isRtl }) => {
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 py-24 transition-colors duration-500 font-sans" dir={isRtl ? "rtl" : "ltr"}>
      <div className="max-w-4xl mx-auto px-4">
        <div className="text-center mb-16">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-3xl bg-brand-500/10 dark:bg-brand-500/20 mb-6 shadow-sm border border-brand-500/20 dark:border-brand-500/30">
            <Settings className="w-10 h-10 text-brand-600 dark:text-accent-400" />
          </div>
          <h1 className={`text-4xl md:text-6xl font-black text-slate-900 dark:text-white mb-6 ${isRtl ? 'font-cairo' : 'uppercase tracking-tighter'}`}>
            {t.nav_cookie}
          </h1>
        ...
        <div className="space-y-8">
          <div className="bg-slate-50 dark:bg-slate-900 rounded-[2rem] p-10 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all duration-300">
            <h2 className={`text-2xl font-black text-slate-900 dark:text-white mb-6 ${isRtl ? 'font-cairo' : 'uppercase tracking-tight'}`}>{isRtl ? 'ما هي ملفات تعريف الارتباط؟' : 'What Are Cookies?'}</h2>
          ...
          <div className="bg-slate-50 dark:bg-slate-900 rounded-[2rem] p-10 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all duration-300">
            <h2 className={`text-2xl font-black text-slate-900 dark:text-white mb-8 ${isRtl ? 'font-cairo' : 'uppercase tracking-tight'}`}>{isRtl ? 'كيف نستخدم ملفات تعريف الارتباط' : 'How We Use Cookies'}</h2>
            <div className="space-y-8">
              <div className="border-s-4 border-brand-500 dark:border-accent-500 ps-6">
                <h3 className={`text-lg font-black text-brand-600 dark:text-accent-400 mb-2 ${isRtl ? 'font-cairo' : 'uppercase tracking-wide'}`}>{isRtl ? 'ملفات تعريف الارتباط الضرورية للغاية' : 'Strictly Necessary Cookies'}</h3>
              ...
              <div className="border-s-4 border-brand-500 dark:border-accent-500 ps-6">
                <h3 className={`text-lg font-black text-brand-600 dark:text-accent-400 mb-2 ${isRtl ? 'font-cairo' : 'uppercase tracking-wide'}`}>{isRtl ? 'ملفات تعريف الارتباط التحليلية' : 'Analytics Cookies'}</h3>
                <p className="text-slate-600 dark:text-slate-300 font-medium">
              ...
              <div className="border-s-4 border-brand-500 dark:border-accent-500 ps-6">
                <h3 className={`text-lg font-black text-brand-600 dark:text-accent-400 mb-2 ${isRtl ? 'font-cairo' : 'uppercase tracking-wide'}`}>{isRtl ? 'ملفات تعريف الارتباط التسويقية' : 'Marketing Cookies'}</h3>
                <p className="text-slate-600 dark:text-slate-300 font-medium">
